# 배하영 | Web Publisher → Frontend Developer 🌱

안녕하세요, 4년차 웹 퍼블리셔 배하영입니다.

HTML/CSS/JS 퍼블리싱을 기반으로, 메가스터디 엘리하이·비상교육 등의
교육 콘텐츠 전자저작물부터 기업 브랜드 사이트까지 다양한 실무를 경험해왔습니다.

요즘은 **GSAP 스크롤 인터랙션**과 **React**에 푹 빠져,
"보는 재미가 있는 웹"을 직접 구현하며 프론트엔드로 영역을 넓혀가고 있습니다.

작은 인터랙션 하나도 "왜 이렇게 동작하는지" 이해하고 넘어가는 걸 좋아하고,
막히는 문제는 끝까지 파보는 편이에요. 🔍

<br/>

## 🛠️ Tech Stack

#### Markup & Style
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)
![Sass](https://img.shields.io/badge/Sass-CC6699?logo=sass&logoColor=white)

#### Language & Library
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![jQuery](https://img.shields.io/badge/jQuery-0769AD?logo=jquery&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black)

#### Interaction
![GSAP](https://img.shields.io/badge/GSAP-88CE02?logo=greensock&logoColor=white)
![Lenis](https://img.shields.io/badge/Lenis-000000?logoColor=white)
![Canvas](https://img.shields.io/badge/Canvas-FF6384?logo=html5&logoColor=white)

### Backend & Auth
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)

#### Design & Tools
![Figma](https://img.shields.io/badge/Figma-F24E1E?logo=figma&logoColor=white)
![Photoshop](https://img.shields.io/badge/Photoshop-31A8FF?logo=adobephotoshop&logoColor=white)
![Illustrator](https://img.shields.io/badge/Illustrator-FF9A00?logo=adobeillustrator&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)

<br/>

## 💼 Projects

#### ✨ BHY Portfolio — 인터랙티브 포트폴리오
GSAP · ScrollTrigger · Lenis 기반의 스크롤 인터랙션 포트폴리오입니다.
[🔗 사이트](https://thriving-strudel-602682.netlify.app/)

#### ✨ BHY.UI — UI 컴포넌트 라이브러리
24개의 UI 컴포넌트와 다크모드, JSON 기반 코드뷰어를 직접 구현했습니다.
[🔗 사이트](https://beahayoung.github.io/BHY.UI/)

#### ✨ 영화 검색 앱 — React 학습 프로젝트 (진행 중)
TMDB API를 활용한 영화 검색 앱으로, React를 학습하며 만들고 있습니다.
[🔗 사이트](https://movie-search-react-umber-three.vercel.app/)

<br/>

## 💬 Career

- **모노브레인** (2024.03 ~ 2025.11) · 웹퍼블리셔 — 메가스터디 엘리하이, 비상교육 전자저작물
- **주식회사 메타록** (2022.07 ~ 2023.08) · PD Lab — 홈페이지 디자인·퍼블리싱
- **한약 한뜻** (2020.11 ~ 2021.11) · 고도몰 홈페이지 관리·디자인

<br/>

## 📫 Contact

- 📧 Email : bhy5635@gmail.com
- 🔗 GitHub : [github.com/beahayoung](https://github.com/beahayoung)
<!-- 블로그 준비중  -->
<br/>

> 퍼블리셔의 꼼꼼함과 개발자의 호기심, 둘 다 놓치지 않으려 합니다. 🌿

<br/>

이전에 클론코딩으로 퍼블리싱한 사이트들
<br/>
## 적응형 사이트
- https://beahayoung.github.io/CloneNexon1v1/
- https://beahayoung.github.io/CloneNexon1v2/
- https://beahayoung.github.io/CloneNexon1v3/
- https://beahayoung.github.io/CloneNexon1v4/
- https://beahayoung.github.io/CloneNexon1v5/
## 반응형 사이트
- https://beahayoung.github.io/Clonecafebean/
